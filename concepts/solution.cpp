...
template <typename T>
concept B = std::integral<T> && std::equality_comparable<T>;
...
template <typename T> concept Comparable = requires(const T a, const T b) {
...
template <C T> auto f1(T &t) { return t; }

auto f2(C auto t) { return 2 * t; }

auto f2a(const C auto &t) { return 2 * t; }

template <typename T> auto f3(T &t) requires A<T> { return t; }

template <convertible_to<bool> T> void f4(const T &t) {}

template <Incrementable T> void f5(const T &t) {}

template <typename T> requires convertible_to<T, bool> void f6(const T &t);

template <typename T> requires Incrementable<T> void f7(const T &t);

template <typename T> requires requires(T x) {
    x++;
    ++x;
}
void f8(const T &t);
